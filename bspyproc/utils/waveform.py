...
class WaveformManager():
    ...
    def format_amplitudes_and_slopes(self, amplitudes_input, amplitude_lengths_input, slope_lengths_input):
    ...
    def safety_format(self, amplitudes, safety_formatting):
    ...
    def generate_waveform(self, amplitudes, amplitude_lengths, slope_lengths=0, safety_formatting=True):
        '''
        ...
        '''
        output = np.ndarray([])
        amplitudes = self.safety_format(amplitudes, safety_formatting)
        amplitudes, amplitude_lengths, slope_lengths = self.format_amplitudes_and_slopes(amplitudes, amplitude_lengths, slope_lengths)
    ...
    def generate_mask(self, amplitudes, amplitude_lengths, slope_lengths=0, safety_formatting=True):
        '''
        Generates a waveform (voltage input over time) with constant intervals of value amplitudes[i] for interval i of length[i].
    ...
    def generate_waveform_from_masked_data(self, x, amplitude_lengths, slope_lengths):
        amplitudes = self.remove_unmasked_platos(x, amplitude_lengths)
        return self.generate_waveform(amplitudes, amplitude_lengths, slope_lengths)

    def remove_unmasked_platos(self, x, amplitude_lengths):
        i = 0
        amplitudes = np.array([])
        while i < len(x):
    ...
    def generate_slopped_plato(self, slope_length, total_length, value=1):
