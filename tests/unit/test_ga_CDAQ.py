...
class GA_Test_CDAQ(unittest.TestCase):
    ...
    The required keys have to be defined in the self.get_configs_CDAQ() function.
    ...
    def get_train_parameters(self):
        ...
        results = {}
        threshold = 30
        size = torch.randint(15, threshold, (1, 1)).item()
        results["inputs"] = TorchUtils.format(torch.rand((size, 2))) / 50
        results["targets"] = TorchUtils.format(torch.randint(0, 2, (size, 1)))
        results["norm_inputs"] = results["inputs"]
