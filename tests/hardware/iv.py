...
class IVMeasurement():
    ...
    def gen_input_wfrm(self):
        def generate_sawtooth(v_high, v_low, n_points, direction):
            ...
            Input[len(Input1) + len(Input2):len(Input1) + len(Input2) +
                  len(Input3)] = Input3
        ...
        if self.input_signal['input_signal_type'] == 'sawtooth':
            input_data = generate_sawtooth(
                self.input_signal['voltage_range'][1],
                self.input_signal['voltage_range'][0], self.configs['shape'],
        ...
        for dev in self.configs['devices']:
            ...
            for i in range(2):
                for j in range(4):
                    ...
                    if j + i * 4 < 7:
                        if self.configs["driver"]['instruments_setup'][dev][
                                "activation_channel_mask"][j + i * 4] == 1:
...
if __name__ == '__main__':
    ...
    configs['driver'] = load_configs(
        'C:/Users/Unai/Documents/programming/brains-py/tests/hardware/brains_ivcurve.yaml'
    )
