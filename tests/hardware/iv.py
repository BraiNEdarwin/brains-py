...
class IVtest(unittest.TestCase):
    ...
    def create_input_arrays(self):
        ...
        for dev in self.configs["devices"]:
            ...
            inputs_dict[dev] = np.zeros(
                (
                    self.configs["processor"]["driver"]["instruments_setup"][dev][
    ...
    def gen_input_wfrm(self):
        def generate_sawtooth(v_high, v_low, n_points, direction):
            ...
            if direction == "up":
                Input1 = np.linspace(
                    0, v_low, int((n_points * v_low) / (v_low - v_high))
                )
                Input2 = np.linspace(v_low, v_high, int(n_points))
                Input3 = np.linspace(
                    v_high, 0, int((n_points * v_high) / (v_high - v_low))
                )
            elif direction == "down":
                Input1 = np.linspace(
                    0, v_high, int((n_points * v_high) / (v_high - v_low))
                )
                Input2 = np.linspace(v_high, v_low, int(n_points))
                Input3 = np.linspace(
                    v_low, 0, int((n_points * v_low) / (v_low - v_high))
                )
            else:
            ...
            Input[
                len(Input1) + len(Input2) : len(Input1) + len(Input2) + len(Input3)
            ] = Input3
        ...
        if self.waveform["input_type"] == "sawtooth":
            input_data = generate_sawtooth(
                self.waveform["V_high"],
                self.waveform["V_low"],
                self.configs["shape"],
                self.waveform["direction"],
            )
        elif self.waveform["input_type"] == "sine":
            input_data = generate_sinewave(
                self.configs["shape"],
                self.configs["processor"]["driver"]["sampling_frequency"],
                self.waveform["V_high"],
            )
            input_data[-1] = 0
        else:
    ...
    def iv_plot(self, configs, output):
        ...
        devlist = configs["processor"]["driver"][
            "instruments_setup"
        ]  # get_default_brains_setup_dict()
        ylabeldist = -5
        ...
        for dev in self.configs["devices"]:
            ...
            for i in range(2):
                for j in range(4):
                    ...
                    if j + i * 4 < 7:
                        if (
                            self.configs["processor"]["driver"]["instruments_setup"][
                                dev
                            ]["activation_channel_mask"][j + i * 4]
                            == 1
                        ):
                        ...
                        axs[i, j].set_title(
                            devlist[dev]["activation_channels"][j + i * 4]
                        )
                    else:
                        axs[i, j].plot(xaxis)
