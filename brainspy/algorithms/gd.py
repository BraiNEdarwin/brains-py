...
def train(
    ...
    for epoch in looper:
        ...
        model, running_loss = default_train_step(model, dataloaders[0], criterion, optimizer, waveform_transforms=waveform_transforms, logger=logger)
        train_losses.append(running_loss)
        description = "Training Loss: {:.6f}.. ".format(train_losses[-1])
        
        if dataloaders[1] is not None and len(dataloaders[1]) > 0:
            val_loss = default_val_step(model, dataloaders[1],criterion, waveform_transforms=waveform_transforms, logger=logger)
