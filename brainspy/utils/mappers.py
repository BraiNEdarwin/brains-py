...
class VariableRangeMapper(nn.Module):
    ...
    def get_offset(self):
        x = self.observer.min_vals - self.observer.max_vals
        v = ( (self.out_max * self.observer.min_vals) - (self.out_min*self.observer.max_vals) )
        return v / x
...
class ExponentialAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.9, ch_axis=0, dtype=torch.quint8,
    ...
    def forward(self, x_orig):
        ...
        if min_vals.numel() == 0 or max_vals.numel() == 0:
            ...
            min_vals = (min_vals * self.averaging_constant) + (self.averaging_constant_opposite * min_vals_cur)
            max_vals = (max_vals * self.averaging_constant) + (self.averaging_constant_opposite * max_vals_cur)
