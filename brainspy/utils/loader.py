...
def load_file(data_dir, file_type):
    if file_type == "pt":
        ...
        raise NotImplementedError(
            f"Loading file from a json file in TorchModel has not been implemented yet. "
        )
...
def info_consistency_check(model_info):
    ...
    if "D_in" not in model_info["processor"]["torch_model_dict"]:
        ...
        print(
            "WARNING: The model loaded does not define the input dimension as expected. Changed it to default value: 7"
        )
    if "D_out" not in model_info["processor"]["torch_model_dict"]:
        model_info["processor"]["torch_model_dict"]["D_out"] = 1
        print(
            "WARNING: The model loaded does not define the output dimension as expected. Changed it to default value: %d."
            % 1
        )
    if "hidden_sizes" not in model_info["processor"]["torch_model_dict"]:
        model_info["processor"]["torch_model_dict"]["hidden_sizes"] = [90] * 6
        print(
            "WARNING: The model loaded does not define the input dimension as expected. Changed it to default value: %d."
            % 90
        )
    return model_info
