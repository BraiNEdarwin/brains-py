...
class CDAQtoNiDAQ(NationalInstrumentsSetup):
    ...
    def __init__(self, configs):
        ...
        Initialize the hardware processor. No trigger source required for this device.
            ...
            output_clipping_range: [float,float]
                ...
                approximately +-4 V. Note that in order to calculate the clipping_range, it needs
                to be multiplied by the amplification value of the setup. (e.g., in the Brains
                setup the amplification is 28.5, is the clipping_value is +-4 (V), therefore,
                the clipping value should be +-4 * 28.5, which is [-110,110] (nA) ). The original
                clipping value of the surrogate models is obtained when running the preprocessing
                of the data in bspysmg.measurement.processing.postprocessing.post_process.
            ...
            amplification: float
                ...
                voltage conversion, as larger signals are easier to detect. In order to obtain the
                real current (nA) output of the device, the conversion is automatically corrected
                in software by multiplying by the amplification value again.
                The amplification value depends on the feedback resistance of each of the setups.
                You can find a guide of the amplification value needed for each setup at the
                brains-py wiki:
            ...
            instruments_setup:
                ...
                activation_voltage_ranges: list
                    ...
                    E.g., [[-1.2, 0.6], [-1.2, 0.6], [-1.2, 0.6], [-1.2, 0.6], [-1.2, 0.6], [-0.7,
                    0.3], [-0.7, 0.3]]
                ...
                readout_channels: [2] list
                    Channels for reading the output current values. The channels can be checked in
                    the schematic of the DNPU device.
                trigger_source: str
                    For synchronisation purposes, sending data for the activation voltages on one
                    NI Task can trigger the readout device of another NI Task. In these cases,
                    the trigger source name should be specified in the configs. This is only
                    applicable for CDAQ to CDAQ setups (with or without real-time rack).
                    E.g., cDAQ1/segment1 - More information at:
                    https://nidaqmx-python.readthedocs.io/en/latest/start_trigger.html

            -------------------------------------------------------------------------------------------------------------------------------------------------
            Appart from these values, there are some internal keys that are added internally during
            the initialisation of the drivers. These are not required to be passed on the configs.

            offset : int
                Value (in milliseconds) that the original activation voltage will be
                displaced, in order to enable the spiking signal to reach the nidaq setup.
                It will be defined by SYNCHRONISATION_VALUE * sampling_frequency.

            auto_start : bool
                If the task is not explicitly started with the DAQmx start_task
                method, it will start it anyway. This value is set to False for this setup.

            max_ramping_time_seconds : int
                To set the ramp time for the setup. It is defined with the flags
                CDAQ_TO_NIDAQ_RAMPING_TIME_SECONDS that can be found in
    ...
    def forward_numpy(self, y):
        ...
        ----------
        y : np.array
            Input data to the forward pass

        Returns
        -------
        np.array
            Resulting output from sending the input data into the device.
        """
        y = y.T
    ...
    def synchronise_input_data(self, y):
        ...
        -------
        np.array
            Synchronized input data based on the offset value, where the synchronisation spike should
            have been received.
        """
